...
$("#sellButton").click(function(){
	...
	$.ajax({
		type: "POST",
		url: "http://192.168.0.148:8081/gme-web/api/v1/otc/busBuyAndSell.json", 
...
function showTicket(){
	$.ajax({
		type: "GET",
		url: "http://192.168.0.148:8081/gme-web/api/v1/kline/ticker.json",
		dataType: "json",
		data: "symbol=usdt",
		success: function(data){
			if (data.code == 200) {
...
function showDeity(type){
	$.ajax({
		type: "GET",
		url: "http://192.168.0.148:8081/gme-web/api/v1/otc/deityList.json",
		...
		success: function(data){
			if (data.code == 200) {
				...
				for (var i = 0;i <dataChar.length; i++) {
					...
					if(type == 1) {
						typePre = "买入";
						typeSuf = "<a class='sybg_2_lv' style='display: inline-block;margin-left: 40px;'>卖出</a>";
					}
					else if(type == 2) {
						typePre = "卖出";
						typeSuf = "<a class='sybg_2_hong' style='display: inline-block;margin-left: 40px;'>买入</a>";
					}
				
					$("#deityTable").append("<tr><td>"+typePre+"</td><td>"+data.orderPrice+"</td><td>"+data.orderNumber+"</td><td>"+data.orderAmount+"</td><td>"+data.orderMinAmount+"-"+data.orderMaxAmount+"</td><td>商家姓名</td><td>888</td><td align='center'>"+stradd+"</td><td>"+typeSuf+"</td><input type='hidden' value='"+data.orderId+"'></input><input type='hidden' value='"+payMethod+"'></input></tr>");
				...
				$("#deityTable tr:not(:first)").each(function(){
					$(this).children().eq(-3).children().click(function(){
						...
						$.ajax({
							type: "POST",
							url: "http://192.168.0.148:8081/gme-web/api/v1/otc/userBuySell.json",
							dataType: "json",
...
function userOrderQuery(status){
	$.ajax({
		type: "GET",
		url: "http://192.168.0.148:8081/gme-web/api/v1/otc/userOrderHistory.json",
		dataType: "json",
		data: "status=" + status,
		success :function(data) {
			if (data.code == 200) {
				...
				for (var i = 0; i < dataList.length; i++) {
					...
					if (data.type == 1) {
						typeStr = "<td class='sybg_2_hong'>买入</td>";
					}else if (data.type == 2){
						typeStr = "<td class='sybg_2_lv'>卖出</td>";
					}
					
					
					$("#userOrderQuery").append("<tr><td>"+data.entryOrderId+"<img src='img/L_30.png' style='margin-left: 10px;'/></td>"+typeStr+"<td>"+data.price+"</td><td>"+data.number+"</td><td>"+data.amount+"</td><td>"+statusStr+"</td><td>"+data.targetName+"</td><td>"+data.createTime+"</td><td>"+operation+"</td></tr>");
				}
				...
				$("#userOrderQuery tr:not(:first)").each(function(){
					...
					$(this).children().eq(-1).children().click(function(){
						$.ajax({
							type: "POST",
							url: "http://192.168.0.148:8081/gme-web/api/v1/otc/userCancleOrder.json",
...
//									$(this).parent().after("<tr><td>aaaa").next().append("</td><td>bbbbb").append("</td><td>ccc</td></tr>");
									...
									$(this).parent().after("<tr id='L_40_tr' style='height: 0px;'><td colspan='9'><div class='L_78_3' id='L_78_3'><div class='L_78_1'>" +
											"<fieldset class='L_78_1_1_1'><legend>请选择以下某种方式向卖家付款</legend>"+
											...
											"</span></p>"+payStr+"<p class='L_78_2_2'>" +
...
function busOrderList(){
	$.ajax({
		type: "GET",
		url: "http://192.168.0.148:8081/gme-web/api/v1/otc/busOrderList.json",
		data: "symbol=usdt",
		dataType: "json",
		success: function(data){
			if (data.code == 200) {
				...
				$("#busDeityTable tr:not(:first)").each(function(){
					...
					if (cancle != "撤销") {
						...
						$(this).children().eq(-2).children().click(function(){
							$.ajax({
								type: "POST",
								url: "http://192.168.0.148:8081/gme-web/api/v1/otc/busCancleOrder.json",
	...
	function busOrderQuery(){
		...
		$.ajax({
			type: "GET",
			url: "http://192.168.0.148:8081/gme-web/api/v1/otc/busOrderHistory.json",
			dataType: "json",
			data: "symbol=usdt",
			success :function(data) {
				if (data.code == 200) {
...
//										$(this).parent().after("<tr><td>aaaa").next().append("</td><td>bbbbb").append("</td><td>ccc</td></tr>");
										...
										$(this).parent().after("<tr id='L_40_tr' style='height: 0px;'><td colspan='9'><div class='L_78_3' id='L_78_3'>" +
												"<div class='L_78_1'><fieldset class='L_78_1_1_1'>" +
												...
												"<span id='orderTimeSpan"+orderId+"'>还剩下0分0秒</span></span></p>"+payStr+"<p class='L_78_2_2'>" +
	...
	function userPayFinish(event){
		...
		$.ajax({
			type: "POST",
			url: "http://192.168.0.148:8081/gme-web/api/v1/otc/userPayFinish.json",
			dataType: "json",
	...
	function busCheckFinish(event){
		...
		$.ajax({
			type: "POST",
			url: "http://192.168.0.148:8081/gme-web/api/v1/otc/busCheckFinish.json",
			dataType: "json",
			data: "payPassword=" + payPassword + "&orderId=" + orderId,
			success: function(data){
				if (data.code == 200) {
	...
	//我要申诉
	function complaint(){
		window.location.href="http://192.168.0.148:8081/gme-web/myWorkList.html";
	}
