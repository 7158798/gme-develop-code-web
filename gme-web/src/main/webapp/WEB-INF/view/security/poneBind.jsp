...
<script type="text/javascript">
	...
	<div class="main_content acc-m-con">
			...
			<div class="m_title"><h4>手机短信验证设置</h4></div>
		
			<div class="sectioncont deposit-sec">
				<p>您的手机短信验证功能已经启用，手机尾号为<b>17</b>。登录、提现等操作时使用。</p>
				<p>注意：<b>启用后不能再修改手机号码</b>。</p>
			</div>
...
$(document).ready(function (e) {
	//var url=window.location.href;
	//if(url.indexOf('?bind_phone') > -1){
	//	$(".deposit-sec").before("<h3 class='bind-phone'>您尚未绑定手机，请先绑定手机！</h3>")
	//}

	var country_code = $('select[name="country_code"]');
	function onCountryChange() {
		var code = country_code.val();
		$('#code_tip').html("+"+code);
		$('#phone_number').val('').focus();
	}
	country_code.change(onCountryChange);
	...
	function phoneCheck(phone) {
		...
		if (phone == '') {
			phoneNum.click(function () {
				$(this).parent().removeClass("err-item");
				$(this).next('p').text("");
			}).next('p').text("手机号码不能为空").parent().addClass("err-item");
			return false;
		} else {
		    if($("#code_tip").text()=='+86'){
                if ( !reg_phone.test(phone) ) {
                    phoneNum.focus().click(function () {
                        $(this).parent().removeClass("err-item");
                        $(this).next('p').text("");
                    }).next('p').text("手机号码不正确，请检查后重新输入").parent().addClass("err-item");
                    return false;
                } else {
                    $("#voicebtn,#smsbtn").prop('disabled', false);
                    return true;
                }
            } else {
                $("#voicebtn,#smsbtn").prop('disabled', false);
	...
	function smsCheck() {
		...
		if(smsVal != ''){
			...
			$('#voicebtn').next("p").text("短信验证码不能为空");
		}
	}
	...
	$("#phone_number").on('input propertychange', function () {
		...
		if(phone.length > 6){
			...
			$("#voicebtn,#smsbtn").prop('disabled', true);
		}
		$(this).parent().removeClass("err-item");
		$(this).next('p').text("");
		if(sendTimes == 1){
			$('#smsbtn').text("获取短信验证码");
			$('#voicebtn').text("获取电话语音验证码").next("p").text("");
		}
	}).blur(function () {
		var phone = $('#phone_number').val();
		phoneCheck(phone);
    ...
    $('#smsbtn').click(function() {
		...
		var phoneInput=$('#phone_number'), country=$("#country_code");
		var phone = phoneInput.val();
		var realname = $('#realname').val();
		if (realname == '') {
			$('#voicebtn').next("p").text("姓名不能为空");
			return;
		}
		...
		if(phoneCheck(phone)) {
			$("#voicebtn,#smsbtn").prop('disabled', true);
			$("#subbtn").addClass("timing");

			var timer = $.timer(function () {
				currentTime = currentTime - 1;
				self.text(currentTime + '秒后可重发');
			...
			var msg = "";
			$.ajax({
				type: "post", url: "/sms/" + smsid, data: json_req,
				success: function (data, textStatus) {
					var json_res = data;
					if (json_res && json_res.result) {
						self.text(currentTime + '秒后可重发');
						timer.play();
						msg = lang == "en" ? "The SMS code has been sent to your cellphone." : "短信验证码" + json_res.msg;
					...
					$("#voicebtn").next("p").html(msg);
				},
				error: function () {
					//resetTimer();
					timer.play();
					$("#voicebtn").next("p").text("网络错误！请稍后重试");
				},
				complete: function () {
				}
			});
    ...
    $('#voicebtn').click(function() {
		...
		if(phoneCheck(phone)) {
			...
			var msg = "";
			$.ajax({
				type: "post", url: "/voiceverify/" + smsid, data: json_req,
				success: function (data, textStatus) {
					var json_res = data;
					if (json_res && json_res.result) {
						self.text(currentTime + '秒后可重发');
	...
	$("#subbtn").click(function () {
		...
		if(reg.test(realname)){
            $('#voicebtn').next("p").text("姓名格式不正确");
            return;
		}

		var pass = $('#fundpass').val();
		if (pass == '') {
			$('#voicebtn').next("p").text("资金密码不能为空");
			return;
		}
		var phone_number = $('#phone_number').val();
		if (phone_number == '') {
			$('#voicebtn').next("p").text("手机号码不能为空");
			return;
		}
		...
		if($(this).hasClass("timing")){
			if( smsCheck() ) {
				$("#bindPhone").submit()
			}
		} else {
			var phone = $('#phone_number').val();
			if( phoneCheck(phone) && smsCheck() ) {
				$("#bindPhone").submit()
			}
		}
	})
...
<div class="footer">
    ...
    <div class="fnav">
        ...
        <div class="tail">
            ...
            <div class="vol-all">
                <span>成交量:</span>
				<span> USDT : $<span id="usdtAll">132933143</span> </span>
                <span> BTC : ฿<span id="btcAll">653</span> </span>
                <span> LTC : Ł<span id="ltcAll">2382</span> </span>
                <span> ETH : E<span id="ethAll">31921</span> </span>
            </div>
            <span id="runTime">
                <!--<a href="#">gate.io</a> is powered by <a href="#">gate.io Dev Team</a>-->
                <span class="admin-view"> Elapsed:7.444ms - cnst:0.9;14:0.3;12:0.4;13:1.5; .234 &nbsp; a/e/r </span>
            </span>
        </div>
    </div>
</div>
...
<script>
    ...
    $("#usdtAll").text(toThousands(132933143));
    $("#btcAll").text(toThousands(653));
    $("#ltcAll").text(toThousands(2382));
    $("#ethAll").text(toThousands(31921));
